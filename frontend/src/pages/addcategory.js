...
const AddCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Basic validation
    if (!category.name || !category.description || !category.careInstruction || !category.noOfplants) {
    ...
    formData.append("noOfplants", category.noOfplants);
    if (category.photo) {
      formData.append("photo", category.photo);
    }

    try {
      const response = await axios.post("http://localhost:4000/categories/add", formData, {
  ...
  return (
    ...
    <div className="add-category-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input type="number" name="noOfplants" value={category.noOfplants} onChange={handleChange} required />
